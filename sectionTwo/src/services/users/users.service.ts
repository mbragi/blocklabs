...
class UserService {
	...
	public async UpdateAuthenticatedUser(userId: string, userData: UpdateUserDto) {
		if (isEmpty(userData)) throw new HttpException(400, 'userData is empty');
		...
		const updateUserById: User = await this.users.findByIdAndUpdate(userId, userData, { new: true, lean: true });
		if (!updateUserById) throw new HttpException(409, "User doesn't exist");
